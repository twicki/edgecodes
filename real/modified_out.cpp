...
std::array<int, 3> compute_global_offsets(int MPIRank, int xCols, int yCols,
                                          const gridtools::clang::domain& dom_) {
  int size = xCols * yCols * 6;
  int faceless = MPIRank % (xCols * yCols);
  int row = faceless / xCols;
  int col = faceless % yCols;
  std::array<int, 3> retval;
  retval[0] = i + col * (dom_.isizie() - dom.iplus());
  retval[1] = j + col * (dom_.jsizie() - dom.jplus());
  retval[2] = 0;
  return retval;
}

bool check_offset(int min, int max, int val) { return (min < val && val < max); }

class test {
private:
  struct stencil_17 {
    // Members
    std::array<int, 2> stage_0_global_i_indices{0, 1};
    std::array<int, 3> global_offsets;
  ...
  public:
    stencil_17(const gridtools::clang::domain& dom_, int rank, int xcols, int ycols) : m_dom(dom_) {
      global_offsets = compute_global_offsets(rank, xcol, ycol, dom);
    }
    ...
    void run(storage_ijk_t& in_, storage_ijk_t& out_) {
      ...
      out_.sync();
      {
        ...
        for(int k = 0 + 0;
          ...
          for(int i = m_dom.iminus() + 0; i <= m_dom.isize() - m_dom.iplus() - 1 + 0; ++i) {
            for(int j = m_dom.jminus() + 0; j <= m_dom.jsize() - m_dom.jplus() - 1 + 0; ++j) {
              if(check_offset(stage_0_global_i_indices[0], stage_0_global_i_indices[1],
                              global_offsets[0] + i)) {
...
public:
  ...
  test(const gridtools::clang::domain& dom, int rank, int xcols, int ycols)
      : m_stencil_17(dom, rank, xcols, ycols) {}
